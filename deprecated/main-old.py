...
def generate_persona(customer: CustomerInfo):
    ...
    # Step 1: Convert customer data into a structured text summary
    text_summary = customer_text(customer)

    # Step 2: Create final prompt
    prompt = create_persona_prompt(text_summary)

    # Step 3: Build LCEL chain: prompt → LLM
    chain = prompt | llm

    # Step 4: Run chain with Langfuse callbacks
    result = chain.invoke(
        text_summary,
